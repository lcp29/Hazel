...
namespace Aster
{
    class GlobalSettingRegistry
    {
      ...
      private:
        template <typename T> static std::string ToStoredString(const T& value);

        template <typename T> static bool TryParseStoredString(const std::string& storedValue, T& outValue);
    ...
    template <typename T> T GlobalSettingRegistry::Get(const std::string& key, const T& defaultValue) const
    {
        ...
        T value{};
        if (!TryParseStoredString(it->second, value)) { return defaultValue; }

        return value;
    }

    template <typename T> std::string GlobalSettingRegistry::ToStoredString(const T& value)
    {
        if constexpr (std::is_same_v<T, std::string>) { return value; }
        else if constexpr (std::is_same_v<T, const char*>) { return value ? std::string(value) : std::string(); }
        else if constexpr (std::is_same_v<T, bool>) { return value ? "true" : "false"; }
        else if constexpr (std::is_integral_v<T>) { return std::to_string(value); }
    ...
    template <typename T> bool GlobalSettingRegistry::TryParseStoredString(const std::string& storedValue, T& outValue)
    {
        ...
        else if constexpr (std::is_enum_v<T>)
        {
            ...
            if (!TryParseStoredString(storedValue, underlyingValue)) { return false; }
        ...
        else if constexpr (std::is_integral_v<T>)
        {
            try
            {
                ...
                if (idx != storedValue.size()) { return false; }
