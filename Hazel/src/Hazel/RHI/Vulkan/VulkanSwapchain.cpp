...
namespace Hazel
{
    namespace
    {
        vk::SurfaceFormatKHR ChooseSurfaceFormat(const std::vector<vk::SurfaceFormatKHR>& formats, RHIFormat format)
        {
            ...
            for (const auto& preferredFormat : preferredSurfaceFormats)
            {
                ...
                {
                    if (surfaceFormat.format == preferredFormat) { return surfaceFormat; }
                }
            }
        ...
        vk::PresentModeKHR ChoosePresentMode(const std::vector<vk::PresentModeKHR>& presentModes, RHISwapchainMode mode)
        {
            ...
            {
                if (presentMode == requestedMode) { return presentMode; }
            }
        ...
        , m_Device(deviceOwner->GetHandle())
    {
        ...
        if (surfaceFormats.empty() || presentModes.empty()) { return; }
        ...
        if (imageUsage == vk::ImageUsageFlags()) { imageUsage = vk::ImageUsageFlagBits::eColorAttachment; }
        ...
        for (uint32_t i = 0; i < m_ImageCount; i++)
        {
            ...
            {
                if (imageAvailableSemaphore) { m_Device.destroySemaphore(imageAvailableSemaphore); }
                if (presentSemaphore) { m_Device.destroySemaphore(presentSemaphore); }
                return;
            }
    ...
    RHI_VK_FUNC_IMPL(RHISwapchain, ~RHISwapchainImpl)() { Release(); }
    ...
    RHISwapchainAcquireResult RHI_VK_FUNC_IMPL(RHISwapchain, AcquireImage)(uint64_t timeout)
    {
        ...
        if (!presentQueue || !presentQueue->IsValid()) { return {}; }
        ...
        auto acquireResult = m_Device.acquireNextImageKHR(m_Swapchain, timeout, acquireSemaphore, VK_NULL_HANDLE);
        ...
        if (!availableSyncPoint.valid) { return {}; }
    ...
    {
        if (!m_IsValid || !m_DeviceOwner || !m_Swapchain || frameNumber >= m_PresentSemaphores.size()) { return false; }

        auto* presentQueue = m_DeviceOwner->GetQueue(RHIQueueType::Present);
        if (!presentQueue || !presentQueue->IsValid()) { return false; }

        const auto presentSemaphore = m_PresentSemaphores[frameNumber];
        if (!presentQueue->WaitSyncPointsAndSignalBinary(waitSyncPoints, presentSemaphore)) { return false; }
    ...
    {
        if (!m_IsValid && !m_Swapchain) { return; }

        auto* deviceOwner = m_DeviceOwner;
        ReleaseWithoutUnregister();

        if (deviceOwner && !m_IsDetached) { deviceOwner->UnregisterSwapchain(this); }
    }
    ...
    void RHI_VK_FUNC_IMPL(RHISwapchain, ReleaseImmediateWithoutUnregister)()
    {
        ...
        for (auto& image : m_Images)
        {
            if (image) { image->ReleaseImmediateWithoutUnregister(); }
        }
        m_Images.clear();
        m_ImageViews.clear();
