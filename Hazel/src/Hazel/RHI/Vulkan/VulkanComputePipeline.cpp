// Implements the Vulkan compute pipeline backend.
// Created: 2026-03-16.
...
namespace Aster
{
                                                                 ...
                                                                 const RHIComputePipelineDesc& desc)
    {
        ...
        if (!m_DeviceOwner || !m_Device || !desc.computeShader) { return; }
    ...
    RHI_VK_FUNC_IMPL(RHIComputePipeline, ~RHIComputePipelineImpl)() { Release(); }
    ...
    void RHI_VK_FUNC_IMPL(RHIComputePipeline, Release)()
    {
        ...
        if (deviceOwner && !m_IsDetached) { deviceOwner->UnregisterComputePipeline(this); }
    }
