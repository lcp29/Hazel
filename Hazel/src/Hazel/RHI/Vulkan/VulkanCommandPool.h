...
namespace Hazel
{
    ...
    public:
        ...
        RHICommandBuffer *CreateCommandBuffer(const RHICommandBufferDesc &desc);
        void Release();
        void ReleaseImmediate();
        ~RHICommandPoolImpl();

        const RHICommandPoolDesc &GetDesc() const { return m_Desc; }
        vk::CommandPool GetHandle() const { return m_CommandPool; }
    ...
    private:
        ...
        RHICommandPoolImpl(RHIDevice *deviceOwner, vk::Device device, const RHICommandPoolDesc &desc, uint32_t queueFamilyIndex);
