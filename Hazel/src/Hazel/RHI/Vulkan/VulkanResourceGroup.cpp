...
namespace Hazel
{
    ...
    bool RHI_VK_FUNC_IMPL(RHIResourceGroup, WriteBuffer)(uint32_t slot,
                                                         RHIBuffer *buffer,
                                                         uint64_t offset,
                                                         uint64_t range,
    ...
    bool RHI_VK_FUNC_IMPL(RHIResourceGroup, WriteSampler)(uint32_t slot,
                                                          RHISampler *sampler,
                                                          uint32_t arrayElement)
    {
        const auto *bindingDesc = FindBinding(slot);
    ...
    {
        if (m_Device && m_HeapOwner && m_HeapOwner->GetHandle() && m_DescriptorSet)
        {
