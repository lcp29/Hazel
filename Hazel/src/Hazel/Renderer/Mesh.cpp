...
namespace Hazel
{
    namespace
    {
        ...
        bool TryGetSharedEdgeCornerIndex(const ObjTriangle& triangle, uint32_t positionNodeIndex, uint32_t& outLocalCorner)
        {
        ...
        Mesh BuildMeshletExpandedMesh(const std::vector<Vertex>& sourceVertices,
                                      const std::vector<FinalTriangle>& finalTriangles,
                                      const std::vector<MeshletBuildState>& meshletBuildStates)
        {
            ...
            for (const auto& meshletBuildState : meshletBuildStates)
            {
                ...
                for (const uint32_t triangleIndex : meshletBuildState.triangles)
                {
                    ...
                    for (const uint32_t sourceVertexIndex : triangle.vertexIndices)
                    {
                        ...
                        if (inserted)
                        {
                            ...
                            meshletVertices.push_back(sourceVertices[sourceVertexIndex]);
                        }
                ...
                meshlets.push_back({indexStart, static_cast<uint32_t>(meshletIndices.size()) - indexStart});
            }

            return Mesh(meshletVertices, meshletIndices, meshlets);
        }
    } // namespace
    ...
    Mesh Mesh::CreateFromObj(std::filesystem::path filePath,
                             const bool generateMeshlets,
                             const uint32_t maxMeshletVertices,
                             const uint32_t maxMeshletIndices)
    {
        ...
        {
            HZ_CORE_ERROR("Failed to load OBJ file '{}': {}", filePath.string(), err);
            return {};
        }

        if (!warn.empty())
        {
            HZ_CORE_WARN("Warnings while loading OBJ file '{}': {}", filePath.string(), warn);
        }
        ...
        std::vector<uint8_t> visitedCorners(corners.size(), 0); // bytemap for duplicated corners
        ...
        auto meshletBuildStates =
            BuildMeshlets(finalTriangles, finalTriangleAdjacency, maxMeshletVertices, maxMeshletIndices);
        if (meshletBuildStates.empty())
        {
            return Mesh(finalVertices, finalIndices);
        }

        return BuildMeshletExpandedMesh(finalVertices, finalTriangles, meshletBuildStates);
    }
} // Hazel
